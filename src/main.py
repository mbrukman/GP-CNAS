...
from src.model import TFModelConvert
from src.tree import GPTree, Individual, Generic
from src.logger import StatWriter
...
class Search:
    def __init__(
        ...
        :param model: Object to convert a tree into a Keras Model
    ...
    def common_eval(self, individual: Individual) -> float:
        ...
        """
        _, acc = self.model.fit_model(individual)
    ...
    def statistics(self, fitness_values: list[float]):
        ...
        StatWriter.accumulate_stat(min_ind, max_ind, mean, std)
        StatWriter.print_global_statistics()
    ...
    def search_loop(self):
        ...
        for g in range(1, self.gen + 1):
            ...
            best_individual = self.eval_save(offspring, best_individual)
...
def main():
    ...
    # (since it exists in tensorflow-datasets)
    model = TFModelConvert(
        Pipeline("cifar10"),
        EPOCHS,
    ...
    search = Search(model, GENERATIONS, N_POP, MAX_DEPTH)
